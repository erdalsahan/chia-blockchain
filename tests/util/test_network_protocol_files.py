...
def test_protocol_bytes() -> None:
    ...
    message_bytes, input_bytes = parse_blob(input_bytes)
    message_62 = type(coin_state_filters).from_bytes(message_bytes)
    assert message_62 == coin_state_filters
    assert bytes(message_62) == bytes(coin_state_filters)

    message_bytes, input_bytes = parse_blob(input_bytes)
    message_63 = type(request_add_puzzle_subscriptions).from_bytes(message_bytes)
    assert message_63 == request_add_puzzle_subscriptions
    assert bytes(message_63) == bytes(request_add_puzzle_subscriptions)

    message_bytes, input_bytes = parse_blob(input_bytes)
    message_64 = type(respond_add_puzzle_subscriptions).from_bytes(message_bytes)
    assert message_64 == respond_add_puzzle_subscriptions
    assert bytes(message_64) == bytes(respond_add_puzzle_subscriptions)
    ...
    message_bytes, input_bytes = parse_blob(input_bytes)
    message_67 = type(request_add_coin_subscriptions).from_bytes(message_bytes)
    assert message_67 == request_add_coin_subscriptions
    assert bytes(message_67) == bytes(request_add_coin_subscriptions)

    message_bytes, input_bytes = parse_blob(input_bytes)
    message_68 = type(respond_add_coin_subscriptions).from_bytes(message_bytes)
    assert message_68 == respond_add_coin_subscriptions
    assert bytes(message_68) == bytes(respond_add_coin_subscriptions)

    message_bytes, input_bytes = parse_blob(input_bytes)
    message_69 = type(request_remove_coin_subscriptions).from_bytes(message_bytes)
    assert message_69 == request_remove_coin_subscriptions
    assert bytes(message_69) == bytes(request_remove_coin_subscriptions)

    message_bytes, input_bytes = parse_blob(input_bytes)
    message_70 = type(respond_remove_coin_subscriptions).from_bytes(message_bytes)
    assert message_70 == respond_remove_coin_subscriptions
    assert bytes(message_70) == bytes(respond_remove_coin_subscriptions)

    message_bytes, input_bytes = parse_blob(input_bytes)
    message_71 = type(request_reset_subscriptions).from_bytes(message_bytes)
    assert message_71 == request_reset_subscriptions
    assert bytes(message_71) == bytes(request_reset_subscriptions)

    message_bytes, input_bytes = parse_blob(input_bytes)
    message_72 = type(respond_reset_subscriptions).from_bytes(message_bytes)
    assert message_72 == respond_reset_subscriptions
    assert bytes(message_72) == bytes(respond_reset_subscriptions)
